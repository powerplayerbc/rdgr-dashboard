// =============================================
// Journal API Layer — Supabase & Webhook Helpers
// =============================================
...
// =============================================
// JOURNAL DRIVE API (n8n webhook)
// =============================================
async function journalDriveApi(operation, data = {}, { silent = false } = {}) {
    if (!activeProfileId) {
        if (!silent) toast('Please select a profile first', 'error');
        return null;
    }
    try {
        const res = await fetch(JOURNAL_DRIVE_URL, {
            method: 'POST',
        ...
        const text = await res.text();
        if (!text) {
            console.warn(`journalDriveApi(${operation}): empty response`);
            return null;
        }
        let result;
        try { result = JSON.parse(text); } catch (e) {
            console.warn(`journalDriveApi(${operation}): non-JSON response:`, text.substring(0, 200));
            return null;
        }
        ...
        console.warn(`journalDriveApi(${operation}) error:`, err.message);
